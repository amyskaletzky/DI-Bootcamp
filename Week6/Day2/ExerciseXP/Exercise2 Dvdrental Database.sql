...
-- SELECT create_date FROM customer --> did this is to get all the dates, but they're all the same date so I used distinct, not sure if thats what was asked for
...
SELECT customer.customer_id	, customer.first_name, customer.last_name, amount, payment_date FROM customer
INNER JOIN payment
ON customer.customer_id = payment.customer_id
INNER JOIN rental
