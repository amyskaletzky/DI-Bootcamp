...
SELECT first_name || ' ' || last_name AS full_name, salary, (0.15 * salary) AS PF FROM employees

-- Write a query to get the employee IDs, names (first_name, last_name) and salary in ascending order according to their salary.
SELECT employee_id, first_name || ' ' || last_name AS full_name, salary FROM employees ORDER BY salary ASC
...
SELECT first_name, last_name, LENGTH(first_name || REPLACE(last_name, ' ', '')) FROM employees

-- Write a query to check whether the first_name column of the employees table contains any numbers.
-- SELECT PG_TYPEOF(first_name) FROM employees
...
SELECT last_name FROM employees WHERE LENGTH(REPLACE(last_name, ' ', '')) = 6;

-- Write a query to display the last name of all employees who have the letter ‘e’ as the third character in the name.
SELECT last_name FROM employees WHERE SUBSTRING(REPLACE(last_name, ' ', ''), 3, 1) ILIKE 'e'
-- SELECT last_name FROM employees WHERE POSITION('e' IN last_name) = 3  --another way
